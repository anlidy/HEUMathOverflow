...
func NewPostHandler(postServ service.PostService) PostHandler {
	return PostHandler{postServ: postServ, name: "Post-Handler"}
}

// 创建帖子
func (pc *PostHandler) CreateNewPost(c *gin.Context) {
...
// 获取帖子信息
func (pc *PostHandler) GetPostData(c *gin.Context) {
	var ctx = c.Request.Context()
...
// 批量获取帖子信息
func (pc *PostHandler) GetManyPostData(c *gin.Context) {
	// 获取偏移量
...
// 更新帖子
func (pc *PostHandler) UpdateOnePost(c *gin.Context) {
	var postIDStr = c.Param("postID")
	postID, err := strconv.ParseInt(postIDStr, 10, 64)
	if err != nil {
		api.JSON(c).Code(http.StatusBadRequest).Message("无效的帖子id").Send()
...
// / 点赞接口
// 点赞帖子
func (pc *PostHandler) LikeOnePost(c *gin.Context) {
	var postIDStr = c.Param("postID")
	postID, err := strconv.ParseInt(postIDStr, 10, 64)
	if err != nil {
		api.JSON(c).Code(http.StatusBadRequest).Message("无效的帖子id").Send()
...
// 取消点赞
func (pc *PostHandler) CancelLikeOnePost(c *gin.Context) {
	var postIDStr = c.Param("postID")
	postID, err := strconv.ParseInt(postIDStr, 10, 64)
	if err != nil {
		api.JSON(c).Code(http.StatusBadRequest).Message("无效的帖子id").Send()
...
// / 收藏接口
// 收藏帖子
func (pc *PostHandler) StarOnePost(c *gin.Context) {
	var postIDStr = c.Param("postID")
	postID, err := strconv.ParseInt(postIDStr, 10, 64)
	if err != nil {
		api.JSON(c).Code(http.StatusBadRequest).Message("无效的帖子id").Send()
...
// 取消收藏
func (pc *PostHandler) CancelStarOnePost(c *gin.Context) {
	var postIDStr = c.Param("postID")
	postID, err := strconv.ParseInt(postIDStr, 10, 64)
	if err != nil {
		api.JSON(c).Code(http.StatusBadRequest).Message("无效的帖子id").Send()
...
// 查询当前用户收藏的帖子（分页）
func (pc *PostHandler) GetUserStarredPosts(c *gin.Context) {
